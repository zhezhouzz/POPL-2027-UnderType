...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        \item Review of strategy algebra.
...
\textbf{Note:} The denotation of $\phi$ ($\denot{\phi}$) is a set of substitutions that satisfy $\phi$; the denotation of ``strategy algebra terms'' is a set of resources.
...
\begin{align*}
    ...
    \textbf{Resource Propositions} \quad p,q ::= & \subExt \phi ~|~ \supExt \phi ~|~ p \land q ~|~ p \lor q ~|~ p \implies q \\
...
\begin{itemize}
    \item $\subExt \phi_1 \land \subExt \phi_2 \dashv\vdash \subExt(\phi_1 \cap \phi_2)$.
    \item $\subExt \phi_1 \oplus \subExt \phi_2 \dashv\vdash \subExt(\phi_1 \cup \phi_2)$.
    \item $\supExt \phi_1 \land \supExt \phi_2 \dashv\vdash \supExt(\phi_1 \cap \phi_2)$.
    \item $\supExt \phi_1 \oplus \supExt \phi_2 \dashv\vdash \supExt(\phi_1 \cup \phi_2)$.
\end{itemize}
\end{lemma}
...
\begin{align*}
    \supExt (0 \leq x \leq 3) * \supExt (0 \leq y \leq x) 
\end{align*}\noindent
...
We interpret terms and qualifiers as atomic propositions in strategy logic.
...
We interpret type annotations as formulae in strategy logic.

\begin{definition}[Type Annotation Interpretation] We interpret $x:\tau$ as:
\begin{itemize}
    \item $\denot{e : \ort{b}{\phi}} \doteq \denot{\vnu = e}\impl \subExt \denot{\phi} $.
    \item $\denot{e : \urt{b}{\phi}} \doteq \denot{\vnu = e}\impl \supExt \denot{\phi} $.
...
\begin{example} Let $\tau_x = \Unit$ and $\tau = \urt{\Int}{1 \leq \vnu \leq 2}$. The denotation of $\denot{f : x{:}\tau_x \sarr \tau}$ is
    ...
    \end{align*}
The $\denot{f : \subExt (x{:}\tau_x \sarr \tau)}$ makes $f$ unconstrained, and $\denot{f : \supExt (x{:}\tau_x \sarr \tau)}$ is unchanged.
 \end{example}
...
\begin{align*}
    ...
    \\\iff&\forall e_x. (\denot{\vnu = e_x} \impl \supExt (1 \leq \vnu \leq 2)) \impl (\denot{\vnu = e_x} \impl \supExt (\vnu = 1))
    \\\iff&\forall e_x. \denot{\vnu = e_x} \impl \supExt (1 \leq \vnu \leq 2)\impl \supExt (\vnu = 1)
\end{align*}
 \end{example}
...
\begin{example}[Situation 1] Concrete example $f \doteq \lambda x. \zif{x == 0}{\Code{int\_gen()}}{\Code{err}}$:
    ...
    \\ \iff &\forall e_x. (\denot{\vnu = e_x} \impl \supExt \Dom{\{x\}}) \wand \denot{(f\;e_x) : \urt{\Int}{\top}}
    \\ \iff &\supExt \Dom{\{x\}} \wand \denot{(f\;x) : \urt{\Int}{\top}}
    \\ \iff &\supExt \Dom{\{x\}} \wand \denot{\zif{x == 0}{\Code{int\_gen()}}{\Code{err}} : \urt{\Int}{\top}}
    \\ \iff &\supExt \Dom{\{x\}} \wand 
    \\ & \quad (\subExt (x = 0) \land \Dom{\{\vnu\}} \oplus  \subExt (x \neq 0) \land \bot) \impl \supExt \Dom{\{\vnu\}}
    \\ \iff &(x = 0 * \Dom{\{\vnu\}}) \models \supExt \Dom{\{\vnu\}} \text{ and } 
    \\ &(\supExt (x \neq 0) * \bot) \models \supExt \bot  
    \end{align*} 
\end{example}
...
\begin{example}[Situation 2] Concrete example $f \doteq \lambda x. x$:
    ...
    \\ \iff &\forall e_x. (\denot{\vnu = e_x} \impl \supExt \Dom{\{x\}}) \impl \denot{e_x : \urt{\Int}{\top}} \land \denot{(f\;e_x) : \urt{\Int}{\top}}
    \\ \iff &\supExt \Dom{\{x\}} \impl \supExt \Dom{\{x\}} \land \denot{(f\;x) : \urt{\Int}{\top}}
    \\ \iff &\supExt \Dom{\{x\}} \impl \supExt \Dom{\{x\}} \land \supExt \Dom{\{x\}}
    \end{align*} 
\end{example}
...
\begin{example}[Situation 2] Concrete example $f \doteq \lambda x. \zif{x == 0}{\Code{int\_gen()}}{\Code{err}}$:
    \begin{align*}
        &f : x{:}\urt{\Int}{\top} \sarr \urt{\Int}{\top}
    \\ \iff &\supExt \Dom{\{x\}} \impl 
    \\ & \quad (\subExt (x = 0) \land \Dom{\{\vnu\}} \oplus  \subExt (x \neq 0) \land \bot) \impl \supExt \Dom{\{x\}} \land \supExt \Dom{\{\vnu\}}
    \\ \iff &(x = 0 \land \Dom{\{\vnu\}}) \models x=0 \land \supExt \Dom{\{\vnu\}} \text{ and } 
