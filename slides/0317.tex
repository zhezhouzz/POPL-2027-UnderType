...
\begin{definition}[Projection]
    ...
    \\&\Code{Proj}(X, m) \doteq
    \begin{cases}
        \bigcup_{\sigma \in m} \{ \Code{Proj}(X, \sigma) \} & \text{when } X \subseteq \Code{Dom}(m) \\
        \text{undefined} & \text{otherwise}
    \end{cases}
    \end{align*}
\end{definition}
...
\begin{align*}
\phi \doteq \Code{Atom} ~|~ \Code{Dom}(X) ~|~ \phi \cap \phi ~|~ \phi \cup \phi ~|~ \phi \setminus \phi ~|~ \phi \times \phi
\end{align*}
where
\begin{itemize}
    \item $\Code{Atom}$ is an atomic proposition, e.g., $\denot{x < 2} = \{[x\mapsto 0], [x\mapsto 1]\}$.
    \item $\Code{Dom}(X)$ ensures that all substitutions have the same domain as $X$.
...
\begin{itemize}
    ...
    \item $m \models \phi$ iff $\{\sigma ~|~ \Code{Dom}(\sigma) = \Code{FV}(p) \land \sigma(p)\}\leq m$.
...
\begin{align*}
    ...
    \textbf{Pure Propositions} \quad \phi ::= & \Code{Atom} ~|~ \Code{Dom}(X)\\
...
\begin{definition}[Monadic Resource]
    ...
    \begin{align*}
        \forall \sigma_0, \Code{Dom}(\sigma_0) \land \Code{Dom}(m(\sigma_0)) 
    \end{align*}
    which means that $m$ cannot produce substitutions with different domain.

    We still defined $\Code{Dom}(m)$ as its output domain.
\end{definition}
...
\begin{example}[Situation 1] Concrete example $f \doteq \lambda x. \zif{x == 0}{\Code{int\_gen()}}{\Code{err}}$:
    ...
    \\ \iff &\forall e_x. (\denot{\vnu = e_x} \impl \lowerB \Code{Dom}(\{x\})) \wand \denot{(f\;e_x) : \urt{\Int}{\top}}
    \\ \iff &\lowerB \Code{Dom}(\{x\}) \wand \denot{(f\;x) : \urt{\Int}{\top}}
    \\ \iff &\lowerB \Code{Dom}(\{x\}) \wand \denot{\zif{x == 0}{\Code{int\_gen()}}{\Code{err}} : \urt{\Int}{\top}}
    \\ \iff &\lowerB \Code{Dom}(\{x\}) \wand 
    \\ & \quad (\upperB (x = 0) \land \Code{Dom}(\{\vnu\}) \oplus  \upperB (x \neq 0) \land \bot) \impl \lowerB \Code{Dom}(\{\vnu\})
    \\ \iff &(x = 0 * \Code{Dom}(\{\vnu\})) \models \lowerB \Code{Dom}(\{\vnu\}) \text{ and } 
...
\begin{example}[Situation 2] Concrete example $f \doteq \lambda x. x$:
    ...
    \\ \iff &\forall e_x. (\denot{\vnu = e_x} \impl \lowerB \Code{Dom}(\{x\})) \impl \denot{e_x : \urt{\Int}{\top}} \land \denot{(f\;e_x) : \urt{\Int}{\top}}
    \\ \iff &\lowerB \Code{Dom}(\{x\}) \impl \lowerB \Code{Dom}(\{x\}) \land \denot{(f\;x) : \urt{\Int}{\top}}
    \\ \iff &\lowerB \Code{Dom}(\{x\}) \impl \lowerB \Code{Dom}(\{x\}) \land \lowerB \Code{Dom}(\{x\})
    \end{align*} 
\end{example}
...
\begin{example}[Situation 2] Concrete example $f \doteq \lambda x. \zif{x == 0}{\Code{int\_gen()}}{\Code{err}}$:
    \begin{align*}
        &f : x{:}\urt{\Int}{\top} \sarr \urt{\Int}{\top}
    \\ \iff &\lowerB \Code{Dom}(\{x\}) \impl 
    \\ & \quad (\upperB (x = 0) \land \Code{Dom}(\{\vnu\}) \oplus  \upperB (x \neq 0) \land \bot) \impl \lowerB \Code{Dom}(\{x\}) \land \lowerB \Code{Dom}(\{\vnu\})
    \\ \iff &(x = 0 \land \Code{Dom}(\{\vnu\})) \models x=0 \land \lowerB \Code{Dom}(\{\vnu\}) \text{ and } 
