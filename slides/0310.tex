...
\begin{definition}[Denotation] We define $\denot{\phi}$ as the set of resources that model $\phi$.
\begin{itemize}
    ...
    \item $\denot{\true} = \{\textbf{1}\}$;  $\denot{\false} = \{\emptyset\}$
...
\begin{itemize}
    \item $\upperB p \land \upperB q \dashv\vdash \upperB(p \land q)$, thus $\upperB(\upperB p \land \upperB q) \dashv\vdash \upperB(p \land q)$. 
...
\begin{proof}
    \begin{itemize}
        ...
        \item For all possible $m$, we have $m \subseteq \denot{q} \cap m$, and $\denot{q} \cap m \models \upperB q$, thus $m \models \lowerB(\lowerB p \lor \upperB q)$.
    \end{itemize}
\end{proof}
