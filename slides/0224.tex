...
\textbf{Base (Valuation Function):} A world can be represented as a substitution; $\Code{Dom}(\sigma)$ indicates the variables in the substitution.
...
\textbf{Equivalence relation:} There is an equivalence relation $\sim$ over worlds $w_1$ and $w_2$. We require that equivalent worlds have the same domain, that is, $\Code{Dom}(w_1) = \Code{Dom}(w_2)$. The intuition is that only aligned assignments can be grouped together, or we can distinguish the set of variables but don't care about the actual assignment.
...
\begin{itemize}
    \item Minimal world: a world $w$ is minimal for a property $\phi$ iff $\Code{Dom}(w) = \Code{FV}(\phi)$.
...
\begin{itemize}
    ...
    \begin{align*}
        \forall w_1 \in W_1, \forall w_2 \in W_2, \Code{Dom}(w_1) \cap \Code{Dom}(w_2) = \emptyset
    \end{align*}
...
\begin{align*}
    ...
    \denot{x{:}\tau_x \wand \tau}_M &\doteq \{ e ~|~ \forall M'.\; \denot{x : \tau_x}_M.\; \Code{Dom}(M') \cap (\Code{Dom}(M) \cup \Code{FV}(\tau)) = \emptyset \implies 
...
\begin{align*}
    ...
    \\\doteq &\{ e ~|~ \forall M'.\; \denot{\boxtimes (1 \leq x \leq 2)}_M.\; \Code{Dom}(M') \cap (\Code{Dom}(M) \cup \Code{FV}(\tau)) = \emptyset \implies 
