...
This section presents the our strategy logic and choice types. Similar with separation logic, our strategy logic is also based on underline resource algebra, although such an algebra is additionally equipped with the branching operators. In the rest of this section, we will first introduce the preliminary definitions and then present the strategy algebra, strategy logic, and our choice types.
...
\begin{definition}[Strategy algebra]\label{def:strategy-semiring}
  ...
  \end{itemize}
The strategy algebra is a partial commutative semiring equipped with a partial order $\leq$ that satisfies the bifunctoriality property:
  \begin{align*}
...
Easily to know, $(\Res,{+},{\times},\{ \emptyset \})$ and $\leq$ is a strategy algebra.
...
\emph{Basic} strategy logic is parameterized by set of atomic propositions $\mathcal{A}$ and has no approximation or persistency modality. Then, every judgment is \emph{precisely} describes a resource.
...
\begin{definition}[Strategy logic]\label{def:strategy-logic-modalities} Strategy logic additional equiped with three modalities than basic strategy logic, i.e., the overapproximation modality $\subExt$, the underapproximation modality $\supExt$, and the persistency modality $\chpers$.
  \begin{itemize}
...
\end{align*}
Then, all formula in strategy logic can be expressed in approximation normal form, i.e., $\subExt$ and $\supExt$ are only applied to pure propositions.
\end{theorem}

\paragraph{Erase modality}
Write $\denot{\phi} \subseteq \Res$ for the set of resources that satisfy~$\phi$.
Then the $\subseteq$-closures above coincide with the modalities from \autoref{def:strategy-logic-modalities}:
\begin{align*}
...
\begin{enumerate}
  ...
  \item A more intuitive analogue in strategy logic is the type context; however, all (overapproximated) values in the type context are assumed to be non-empty (in CBV semantics).
\end{enumerate}
...
The denotational semantics of a type is given by a function in $\Res \sarr (X \sarr \Res \sarr \Res) \sarr \Code{Prop_{Choice}}$; it takes a context (resource) and a term (via its denotation) and returns a proposition in strategy logic stating whether the term inhabits the type under that context.
\begin{align*}
...
\end{align*}\noindent
The denotation of a typing context is a function in $\Res \sarr \Res \sarr \Code{Prop_{Choice}}$: under a context, it yields a predicate over contexts in strategy logic.
\begin{align*}
...
\begin{definition}[Strategy Logic denotation] In order to simplify the notation, we also define the denotation of the terms in strategy logic, which has the same meaning as the force semantics in strategy logic.
  \begin{align*}
