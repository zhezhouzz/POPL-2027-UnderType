...
\end{align*}\noindent
On the other hand, \textsc{T-LetI} produces an independent
...
\begin{align*}
  &\underline{\empty \ctstar y_1{:}\urt{\Nat}{\top} \vdash \zlet{y_2}{f\;()}{e} : \tau \quad \judgfail}  \tag{\textsc{T-LetI}}
  \\
...
capability containing different choices:
...
\begin{definition}[Persistent Modality] We introduce a persistence
  ...
  least contain all the variables needed by $P$,
...
the following type judgment, which is invalid under \langNDname{}'s
...
\begin{definition}[Persistent Type Denotation]
  ...
  inhabitants like
  $\zzlam{x}{0} \oplus \zzlam{x}{\Code{gen_{nat} () + 1}}$. To ensure
  that $e$ is single-valued, we introduce a universally quantified
  fresh variable $x$ that ranges over all possible reduction results
  of $e$; the persistent modality then ensures both the context and
  this $x$ are single-valued.
\end{definition}

\paragraph{Type rules for persistent types}
...
To illustrate, consider the following typing derivation:
\begin{align*}
\begin{prooftree}
  ...
  \infer2{\emptyset \vdash \Code{gen_{bool} ()} : \ort{\Bool}{\vnu =
...
hold \emph{semantically}, i.e.,
$\not \models~ \denot{\ort{\Bool}{\vnu = \true} \ctoplus
  \ort{\Bool}{\vnu = \false}}~ \Code{gen_{bool} ()} $! The issue here
is that the original denotation of $\choplus$ only splits the
...
provided by a nondeterministic term into the type context:
\begin{align*}
...
the sum operator to partition it across the two branches as usual:
\begin{align*}
