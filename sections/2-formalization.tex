...
\quad
  {\normalsize \begin{flalign*}
    &\text{\textbf{Subtyping rules}} && \fbox{$\Gamma \ctvdash \tau <: \tau$ \quad $\Gamma \ctsubseteq{X} \Gamma$ \quad $\Code{ToOver}(\Gamma, \Gamma')$ }
  \end{flalign*}}
\mprooftr{34mm}
...
\quad
	...
	{CtxOver}
	{$\Code{ToOver}(\Gamma_1, \Gamma_2)$}
	  }
	  \caption{Auxiliary well-formedness and semantic subtyping rules for choice typing. We disallow the contraction rule (duplicate context) for both $\ctcomma$ and $\ctstar$, since the resource will not be consumed but only depended on. The subtyping rules link typing to semantic implication on denotations. $\Code{ToOver}$ is a context-level over-approximation coercion used to type separating let-bindings.}
...
\begin{figure}[h]
  ...
  {$\Gamma \ctvdash \primop : \overline{x_i{:}\tau_i}\ctwand \tau_y$ \quad 
    $\forall i. \Gamma_i \ctvdash v_i : \tau_i$ }
  {T-AppOpD}
  {$\Gamma \ctstar \overline{\Gamma_i} \ctvdash \primop\;\overline{v_i} : \tau[\overline{x_i \mapsto v_i}]$}
  \\[.8em]
  \mprooftr{64mm}
  ...
  \\[.8em]
  \mprooftr{74mm}
  {$\forall i.\;\Gamma_i \ctvdash v : \tau_{vi}$  and $\Dom{\Delta_i} = \{\overline{y_{ij}}\}$ and  $\Delta_i(\Gamma_i) \ctvdash d_i(\overline{y_{ij}}) : \tau_{vi}$ and $\Delta_i(\Gamma_i) \ctvdash e_i : \tau_i$ }
  {T-Match}
  {$\ctbigoplus \overline{\Gamma_i} \ctvdash \match{v} \overline{d_i\ \overline{y_{ij}} \to e_i}  : \ctbigoplus \overline{\tau_i}$}
  }
