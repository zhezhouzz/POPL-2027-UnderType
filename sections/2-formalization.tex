...
This section presents our strategy logic and strategy types. Similar to separation logic, our strategy logic is also based on an underlying resource algebra, although such an algebra is additionally equipped with branching operators. In the rest of this section, we first introduce preliminary definitions and then present the strategy algebra, strategy logic, and strategy types.
...
\begin{definition}[Substitution]\label{def:subst-compat}
 ...
 As mentioned in \autoref{sec:overview}, a \emph{strategy resource} is a nonempty set of substitutions that jointly represent which assignments remain concurrently viable---the proof-relevant ``state of play'' for strategic contexts. Elements of~\(\Res\) below are precisely these strategy resources.

\begin{definition}[Strategy resource]\label{def:strategy-resource}
A strategy resource is a \emph{non-empty} set of substitutions that \emph{share the same domain}. Writing $\Res$ for the collection of such objects, set
\begin{align*}
  \Res \doteq \{ r ~|~ r \neq \emptyset \text{ and } r \in \mathcal{P}(\Code{Var} \rightharpoonup \Code{Value}) \text{ and } \forall \sigma\;\sigma' \in r. \Dom{\sigma} = \Dom{\sigma'} \}.
\end{align*}
We also extend the definitions of domain, compatibility, and projection from substitutions to strategy resources. That is,
\begin{align*}
  &\Dom{r} \doteq  \Dom{\subs} \quad \text{for arbitrary } \subs \in r 
  \\&\mapCompat{r_1}{r_2}
  \doteq
  \forall \subs_1 \in r_1.\;\forall \subs_2 \in r_2.\; \mapCompat{\subs_1}{\subs_2}
  \\&\projectTo{r}{X}
  \doteq
  \{\, \projectTo{\subs}{X} \mid \subs \in r \,\}
\end{align*}
\end{definition}

% \paragraph{\textcolor{red}{New product}} Problem: we cannot show the overapproximation modality is persistent, $\subExt (x > 0) \models \subExt (x > 0) \chstar \subExt (x > 0)$ is wrong. Consider a resource $r \doteq \{[x\mapsto 1], [x\mapsto 2]\}$, we have
% \begin{align*}
%   &\subs_1 \doteq [x\mapsto 1] \in r \quad \subs_2 \doteq [x\mapsto 2] \in r \quad \text{ and } \neg \mapCompat{\subs_1}{\subs_2}
% \end{align*}
% Previous definition of $r_1 \times r_2$ works well in most over situation besides the case when $r_1$ and $r_2$ has the same domain. If they has the same domain, the $\subExt (x > 0) \chstar \subExt (x > 0)$ means singleton resource, which is too restrict. Ideally, the overapproximation modality should be naturally persistent, and persistent means it is independent from the resource. More precisely, we want
% \begin{align*}
%   r \models P \iff \forall r'. r' \subseteq r \implies r' \models P
% \end{align*}\noindent
% where \textcolor{red}{we do overapproximation on the left hand side resource, instead of denotation of $P$}. This intuition make a deep connection over the resource modality (underapproximation) with overapproximation modality. Now, the problem is generalize the definition of $\mapCompat{r_1}{r_2}$ and $r_1 \times r_2$ to satisfy the new intuition. The intuition here is that $P \chstar Q$ should be treat as \textcolor{red}{after I makes a decision on $P$ allowed, then I can still have freedom to make decision on $Q$ allowed, and \textbf{vice versa}}. Previously, we define:
% \begin{align*}
%   r_1 \times r_2 \;\doteq\;
%     \begin{cases}
%       \{\, \subs_1 \cup \subs_2 \mid \subs_1 \in r_1,\; \subs_2 \in r_2 \,\}
%         & \text{if } \forall \subs_1 \in r_1.\;\forall \subs_2 \in r_2.\; \mapCompat{\subs_1}{\subs_2}, \\
%       \text{undefined}
%         & \text{otherwise.}
%     \end{cases}
% \end{align*}\noindent
% which doesn't works for $r \times r$.

Formally, make a decision in our setting is fixing a store from the resource, thus we can define:
\begin{align*}
  r_1 \vec{\times} r_2 \doteq \bigcup_{\sigma_1 \in r_1} \{ \sigma_1 \cup \sigma_2 \mid \mapCompat{\sigma_1}{\sigma_2} \}
\end{align*}\noindent
Assume we have $r_1 \doteq \{[x\mapsto 1], [x\mapsto 2]\}$ and $r_2 \doteq \{[x\mapsto 1], [x\mapsto 2], [x\mapsto 3]\}$. Then we have

Note that for any strategy resource $r$, $\mapCompat{r}{r}$ always holds since they always agree on their domain. Similarly, singleton set $\{r\}$ is compatible with itself.

\begin{definition}[Strategy-resource operations]\label{def:strategy-resource-times} We define two operations ($\Res \times \Res \rightharpoonup \Res$) on strategy resources. The \emph{(strategy) resource product} $r_1 \times r_2$ is the partial binary operation that produces the pairwise union of two strategy resources, i.e.,
  \[
    r_1 \times r_2 \;\doteq\;
    \begin{cases}
      \{\, \subs_1 \cup \subs_2 \mid \subs_1 \in r_1,\; \subs_2 \in r_2 \,\}
    ...
    \end{cases}
  \]
The \emph{(strategy) resource sum} $r_1 + r_2$ is the union of two strategy resources, i.e.,
\[
  r_1 + r_2 \;\doteq\;
  \begin{cases}
    r_1 \cup r_2
      & \text{if } \forall \subs_1 \in r_1.\; \forall \subs_2 \in r_2.\; \Dom{\subs_1} = \Dom{\subs_2}, \\
    \text{undefined}
      & \text{otherwise.}
  \end{cases}
\]
...
The partial order $\leq$ on strategy resources is the analogue of ``more information by releasing the variables set constraint''.

\begin{definition}[Partial order]\label{def:partial-order} The partial order $\leq$ on $\Res$ is defined as 
    \begin{align*}
      r_1 \leq r_2 \doteq \projectTo{r_2}{\Dom{r_1}} = r_1.
    \end{align*}
It is easy to see that $\leq$ is reflexive, antisymmetric, and transitive.
\end{definition}

\begin{definition}[Fibers on strategy resources]\label{def:strategy-resource-fiber}
  Given a substitution $\sigma_X$ and a strategy resource $r$, we write $\fiberOver{r}{\sigma_X}$ for the fiber over $\sigma_X$, i.e., the set of substitutions in $r$ whose projection to $\Dom{\sigma_X}$ equals~$\sigma_X$:
  \begin{align*}
    \fiberOver{r}{\sigma_X} \doteq \{\, \sigma' \mid \sigma' \in r \text{ and } \projectTo{\sigma'}{\Dom{\sigma_X}} = \sigma_X \,\}.
  \end{align*}
\end{definition}

\subsection{Strategy algebra}

Strategy resources below collectively carry algebraic structure used throughout the semantics: the carrier $\Res$ is not only a partial commutative monoid as in separation logic, but also organizes as a partial commutative semiring (PCS).

\begin{definition}[Strategy algebra]\label{def:strategy-semiring}
  ...
  \begin{itemize}
  \item Indentity: $\forall r \in \mathcal{M}. r \times \mathbf{1} = r$.
  \item Commutative (when operations are defined): $\forall r_1\;r_2 \in \mathcal{M}. r_1 + r_2 = r_2 + r_1$ and $r_1 \times r_2 = r_2 \times r_1$.
  \item Associative (when operations are defined): $\forall r_1\;r_2\;r_3 \in \mathcal{M}. (r_1 + r_2) + r_3 = r_1 + (r_2 + r_3)$ and $(r_1 \times r_2) \times r_3 = r_1 \times (r_2 \times r_3)$.
  \end{itemize}
A \emph{strategy algebra} is a partial commutative semiring equipped with a partial order $\leq$ that satisfies the bifunctoriality property:
  \begin{align*}
    \forall r_1\;r_2\;r_1'\;r_2' \in \mathcal{M}. r_1 \leq r_1' \land r_2 \leq r_2' &\implies r_1 \times r_2 \leq r_1' \times r_2' 
    \\& (\text{when } r_1 \times r_2 \text{ and } r_1' \times r_2' \text{ are defined.})
    \\\forall r_1\;r_2\;r_1'\;r_2' \in \mathcal{M}. r_1 \leq r_1' \land r_2 \leq r_2' &\implies r_1 + r_2 \leq r_1' + r_2' 
    \\& (\text{when } r_1 + r_2 \text{ and } r_1' + r_2' \text{ are defined.})
  \end{align*}
  \end{definition}

It is easy to see that $(\Res,{+},{\times},\{ \emptyset \})$ and $\leq$ form a strategy algebra.

\begin{example}[Strategy algebra]
...
$r_x \leq r_{xy}$ and $r_y \leq r_{xy}$.
The product satisfies the same restrictions:
$\projectTo{(r_x \times r_y)}{\{x\}} = r_x$ and
$\projectTo{(r_x \times r_y)}{\{y\}} = r_y$.
Nevertheless $|r_x \times r_y| = 4 > 2 = |r_{xy}|$; equivalently,
\[
  r_x \times r_y
...
\begin{align*}
  \forall r_1\;r_2 \in \Res. r_1 \leq r_1 \times r_2 \quad (\text{when } r_1 \times r_2 \text{ is defined.})
\end{align*}
...
\begin{example}[Strategy algebra]
  ...
  $r_{xy} \doteq \{\, [x \mapsto 1,\, y \mapsto 3],\, [x \mapsto 2,\, y \mapsto 4]\,\}$.
  We have
  $\restrictTo{r_{xy}}{\{x\}} = r_x$ and $\restrictTo{r_{xy}}{\{y\}} = r_y$,
  so by the definition of~$\leq$,
  $r_x \leq r_{xy}$ and $r_y \leq r_{xy}$.
  The product satisfies the same restrictions:
  $\restrictTo{(r_x \times r_y)}{\{x\}} = r_x$ and
  $\restrictTo{(r_x \times r_y)}{\{y\}} = r_y$.
  Nevertheless $|r_x \times r_y| = 4 > 2 = |r_{xy}|$; equivalently,
  \[
    r_x \times r_y
    = r_{xy} + \{\, [x \mapsto 1,\, y \mapsto 4],\, [x \mapsto 2,\, y \mapsto 3] \,\},
...
\emph{Basic} strategy logic is parameterized by a set of atomic propositions $\mathcal{A}$ and has no approximation modalities. Then, every judgment \emph{precisely} describes a resource.
We write $r \models p$ for satisfaction of a resource proposition $p$
at world $r \in \Res$.

\begin{definition}[Basic strategy logic] Let $(\mathcal{M},{+},{\times},\mathbf{1}, \leq)$ to be a strategy algebra. Let $\mathcal{A}$ to be atomic propositions. Let $\denot{-} : \mathcal{A} \sarr \mathcal{M}$ be an interpretation function of pure formulas $\phi$. Then we have:
\begin{itemize}
\item $r \models \chtrue$ always;
\item $r \models \chfalse$ never;
\item $r \models a$ iff $a \in \mathcal{A}$ and $\denot{a} \leq r$;
\item $r \models p \chland q$ iff $r \models p$ and $r \models q$;
\item $r \models p \chlor q$ iff $r \models p$ or $r \models q$;
\item $r \models p \chimpl q$ iff for all $r'$, if $r \leq r'$ and $r' \models p$ then $r' \models q$;
...
\item $r \models p \chstar q$ iff $\exists r_1\;r_2.\; r_1 \times r_2 \leq r$ and
  $r_1 \models p$ and $r_2 \models q$.
\item $r \models p \chwand q$ iff $\forall r'.\; \mapCompat{r'}{r}$ and $r' \models p$ implies
  $r' \times r \models q$.
\item $r \models p \choplus q$ iff $\exists r_1\;r_2.\; r_1 + r_2 \leq r$ and
  $r_1 \models p$ and $r_2 \models q$.
...
\begin{definition}[Strategy logic]\label{def:strategy-logic-modalities} Strategy logic is additionally equipped beyond basic strategy logic with the overapproximation modality $\subExt$, the underapproximation modality $\supExt$, and the fiberwise universal modality $\forallM{\cdot}$.
  \begin{itemize}
...
$\forallM{x_1}\,\forallM{x_2}\,\cdots\,\forallM{x_n}\,p$. The $\forallM{x}p$ means that $p$ holds is independent from the decision made by strategy resource $r$ on variable $x$.
...
\begin{align*}
  &r \models \subExt (0 < x) \chland \supExt (x \leq y < 3) 
  \\\iff& r \models \subExt (0 < x) \text{ and } r \models \supExt (x \leq y < 3)
  \\\implies& \forall \sigma \in r. 0 < \sigma(x) \text{ and } r \models \supExt (x \leq y < 3)
...
% \end{align*}
% Then, all formulas in strategy logic can be expressed in approximation normal form, i.e., $\subExt$ and $\supExt$ are applied only to pure propositions.
% \end{theorem}

\subsection{Core Language and Strategy Types}
...
\begin{figure}[h]
  {
    \begin{alignat*}{2}
      ...
      \\ \text{\textbf{Strategy Types}}& \quad & \tau ::= \quad & \ort{b}{\phi} ~|~ \urt{b}{\phi} ~|~ \tau \ctinter \tau ~|~ \tau \ctunion \tau ~|~ \tau \ctoplus \tau ~|~ x{:}\tau \ctsarr \tau ~|~ x{:}\tau \ctwand \tau
      %  ~|~  \ctind \tau 
      ~|~ \ctpers \tau 
      % ~|~ \ctDiv{s}
  ...
  \end{align*}}
  \caption{Strategy Types Syntax. }
  \label{fig:strategy-types-syntax}
  \end{figure}
...
\begin{definition}[Totality under a strategy resource]\label{def:total-under-resource}
For $r \in \Res$ and $e$ with $\FV(e) \subseteq \Dom{r}$, lift $\Code{Total}(e)$ to strategy logic by:
\[
...
\include{sections/tech/2-basictyping}
...
% 	{$\Gamma_1 \ctstar \Gamma_2 = \Gamma_1$}
	  ...
	  \caption{Auxiliary well-formedness and semantic subtyping rules for strategy typing. We disallow the contraction rule (duplicate context) for both $\ctcomma$ and $\ctstar$, since strategy resources will not be consumed but only depended on. The subtyping rules link typing to semantic implication on denotations. $\Code{ToOver}$ is a context-level over-approximation coercion used to type separating let-bindings.}
...
\begin{align*}
  &\denot{\subExt (0 \leq x \leq 1)} =\{ \{ [x=0] \},  \{ [x=1] \}, \{ [x=0], [x=1] \}, ... \}
  \\&\denot{\chpers \subExt (0 \leq x \leq 1)} =\{ \{ [x=0] \},  \{ [x=1] \}, ... \}
\end{align*}\noindent
...
\begin{align*}
  &\chpers \subExt (0 \leq x \leq 1) \models (x = 0) \chlor (x = 1) \judgok
  \\&\subExt (0 \leq x \leq 1) \models (x = 0) \chlor (x = 1) \judgfail
\end{align*}\noindent
