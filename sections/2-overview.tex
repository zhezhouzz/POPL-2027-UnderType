...
admits worlds in which every environment maps the variable to a value
satisfying $\phi$. An angelic refinement, in contrast, acts as a
\emph{lower} bound: as a result type, $\urt{b}{\phi}$ requires the
image of a term to contain every value satisfying $\phi$; in a type
context, it forces worlds to at least include environments mapping the
variable to each value that satisfies $\phi$. To illustrate, % these
...
\end{align*} %
Observe there are many singleton worlds containing an environment in
which $x$ is assigned a value greater than $2$, but $x - 3$ can only
reach a single result value under such a world!
...
qualifier is a singleton, or when the resulting refinement is trivially
true ($\ort{b}{\top}$) or unsatisfiable ($\urt{b}{\bot}$):
\begin{align*}
...
\begin{align}
  ...
  &\zlet{x}{\Code{nat\_gen () }}{\zlet{y}{\Code{nat\_gen()}}{\square}}
...
position. Existing systems instead fix a single mode of reasoning —
safety or reachability — uniformly across a signature, or otherwise
restrict how dependency between arguments may be expressed.
...
For example:
\begin{align*}
  &x{:}\urt{\Nat}{even(\vnu)} \vdash x + 1 : \ort{\Nat}{odd(\vnu)} \ctoplus
...
to the angelic refinement of $x$ in the typing context from those
that may exist, while the second uses $\ctoplus$ to distinguish
between the two possible control flows through the program.
...
$4$ and another whose result is always less than $4$.\footnote{In this
  sense, the $\ctoplus$  operator plays a role analogous to the commutative
  monoid disjunction in Outcome Logic~\cite{ZDS23}.}   This ability also allows us to
focus on the safety properties of a particular program path, assigning
the remainder type $\top$:
\begin{align*}
