...
singleton angelic type, as it can evaluate to at most one value.
Richer reachability properties naturally arise when reasoning about
the behavior of a term within a broader program context. To do so, our
type system \emph{holistically} considers the \emph{set} of values
that a term can evaluate to, i.e., its image, under a restricted
\emph{set} of environments, or \emph{capability}, that are constrained
by a typing context.
...
\begin{align*}
  x{:}\urt{\Nat}{\vnu > 3} \vdash x - 3 : \urt{\Nat}{\vnu > 0}\;\judgok \\
  x{:}\urt{\Nat}{\vnu > 4} \vdash x - 3 : \urt{\Nat}{\vnu > 0}\;\judgfail
\end{align*} %
\noindent
Clearly, for any particular positive number, the term $x - 3$ will
evaluate to that number, assuming an appropriate assignment to $x$.
The angelic types of $x$ in each typing context place different
restrictions on the set of possible assignments to $x$ in a
capability: under the first, for every $n$ greater than $3$, the
capability must contain an environment in which $x$ is $n$; the second
allows a capability which does not include a mapping of $x$ to $4$,
and thus cannot guarantee the image of $x - 3$ includes $1$.
...
\end{align*} %
The image of $x - 3$ includes a non-positive value in any capability
...
possible to derive meaningful safety guarantees using reachability
...
trivially holds ($\ort{b}{\top}$ or $\urt{b}{\bot}$): %
\begin{align*}
...
\end{align}\noindent
Both contexts use $\Code{gen_{nat} ()}$ to (nondeterministically) generate
an arbitrary value for $x$. The first context then sets $y$ based on
the value of $x$, while the second opts to generate a fresh value for
$y$ instead. The values bound to $x$ and $y$
in either context can all reach every possible positive
number, as can the program \ref{ex:DisjCon}[$x - y$].  This is not
true of \ref{ex:EnCon}[$x - y$], however, because the values of
$x$ and $y$ in this program are \emph{entangled} since the values $y$ can
yield are tied to $x$'s.
\begin{align*}
  ...
  &\emptyset \vdash \text{\ref{ex:EnCon}}[y] : \urt{\Nat}{\vnu > 0}
    \;\judgok \\
  & \emptyset \vdash \text{\ref{ex:DisjCon}}[x -_{\Code{nat}} y] : \urt{\Nat}{\vnu
    > 0} \;\judgok \quad & \emptyset \vdash \text{\ref{ex:EnCon}}[x -_{\Code{nat}} y] : \urt{\Nat}{\vnu > 0} \;\judgfail
\end{align*}\noindent
...
independent of the other context.  Dually, $\Gamma_1 \ctcomma \Gamma_2$
means that the capabilities of $\Gamma_1$ and $\Gamma_2$ are dependent;
an angelic type binding in $\Gamma_2$ is constrained by the assignments
made in $\Gamma_1$.  To illustrate, consider the
following two subtyping judgments reflecting the typing of the \Code{let}-body
in $C_{\mathit{en}}$ and $C_{\mathit{dis}}$, resp:
\begin{align*}
  &x{:}\urt{\Nat}{\vnu > 0} \ctstar y{:}\urt{\Nat}{\vnu > 1}
    \vdash \urt{\Nat}{\vnu = x -_{\Code{nat}} y} \subty \urt{\Nat}{\vnu > 0}
    \;\judgok \\
  &x{:}\urt{\Nat}{\vnu > 0} \ctcomma y{:}\urt{\Nat}{\vnu > 1}
    \vdash \urt{\Nat}{\vnu = x -_{\Code{nat}} y} \subty \urt{\Nat}{\vnu > 0} \;\judgfail
\end{align*}\noindent
Under the first context, we can freely choose both $2$ for $y$
...
for $x$, the qualifier $\vnu > 1$ ascribed to $y$ must hold for
\emph{that same} value of $x$'s capability, rather than for a value
chosen independently. Concretely, the angelic guarantee for $y$ under
``$\ctcomma$'' only promises that $y$ can reach every value satisfying
$\vnu > 1$ \emph{for some fixed instantiation of $x$}.  Since the
subtyping goal requires producing every positive number as $x
-_{\Code{nat}} y$, and this requires pairing each candidate value of
$x$ with an independently chosen $y$, a dependent context cannot be
used to supply such pairs.  This is evidenced by $C_{en}$, which
always collapses its result to 0, rather than all positive numbers.
...
$\Gamma \vdash y{:}\tau_y$ are valid. Importantly, however, it does
not guarantee that the modalities of $x$ and $y$ hold \emph{at the
  same
...
typing context ensures that $x + 1$ must evaluate to some even and
some odd number, without stipulating what those numbers are. In the
second judgment, $\ctoplus$ is used to distinguish between the two
possible control flows through the program.
