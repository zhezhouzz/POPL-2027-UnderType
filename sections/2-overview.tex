...
positive value for $x$ \emph{and} (say) $2$ for $y$, from which the
subtyping relationship follows.  Under the second context, the
dependent conjunction $\ctcomma$ entangles the choices for $x$ and
$y$: once a value is chosen for $x$, the qualifier $\vnu > 1$ ascribed
to $y$ must hold for \emph{that same} value of $x$'s capability,
rather than for a value chosen independently. Concretely, the angelic
...
every positive number as $x -_{\Code{nat}} y$, and this requires
pairing each candidate value of $x$ with an independently chosen $y$,
a dependent context cannot be used to supply such pairs.  This is
evidenced by $C_{en}$, which always collapses its result to 0, rather
than all positive numbers.


%% Intuitively, the hole in \ref{ex:EnCon} has this
%% typing context: once $x$ is fixed, the value of $y$ depends on it, and
%% vice-versa.  As a consequence, we are unable to prove the following
%% typing judgement:
%% \begin{align*}
