...
\label{sec:formalization-details}

In this section, we provide the complete definitions of environments
...
\begin{itemize}
 \item \emph{Domain}: We write $\Dom{\sigma}$ for its domain, i.e., the set of variables where the store is defined.
  \item \emph{Compatibility}: Write $\mapCompat{\sigma_1}{\sigma_2}$ when $\sigma_1$ and $\sigma_2$ are
    ...
    \]
    Note that the concatenation of two environment is defined as a map
    union $\sigma_1 \cup \sigma_2$, which is defined when
