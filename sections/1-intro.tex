...
program reasoning techniques. While their particulars can vary wildly,
common to all type systems are two key ingredients: the use of
\emph{types} to specify program behavior, and some deductive mechanism
for \emph{compositionally verifying} that a program is consistent with
a specification. Type systems have traditionally targeted
\emph{safety} properties, a focus embodied by Milner's famous slogan
that ``well-typed expressions cannot 'go
wrong'''~\cite{Milner+78}. Here, the typing judgement
$\Gamma \vdash e : \tau$ asserts that $e$ \emph{may} evaluate to many
values (under an environment consistent with $\Gamma$), but that set
of values will fall within the ``safe'' region specified by $\tau$.

Dually, \emph{reachability} properties seek to which precisely capture
a subset of the values that a program \emph{must} reach. Rather than
ruling out certain executions, reachability analyses seek to soundly
establish the existence of (often undesirable)
executions. Reachability analyses are a well-established and practical
technique for finding buggy behaviors~\cite{BMC+03}; more recently,
several works~\cite{ReverseHoareLogic, OHP19, ZDS23} have studied
``incorrectness'' systems designed to prove the existence of witnesses
of buggy program behavior, with a goal of avoiding the overwhelming
number of false positives reported by typical bug detection tools.
The question of how to develop type systems targeting reachability
properties, however, has been largely unexplored. A few recent
exceptions~\cite{RefinementTypeRefutations, CoverageType, Ill-types},
have begun to study type-based reachability reasoning, but each of
these systems has focused on a particular property, e.g., checking
coverage properties of test input generators, or applications, e.g.,
generating concrete counterexamples refuting refinement type
specifications.

This paper proposes a new refinement type system in which may- and
must-style specifications coexist as first-class citizens, allowing
safety and reachability specifications to be freely combined within a
unified type discipline. In our type system, type qualifiers are
equipped with a modality indicating whether it should be interpreted
as a may or must specification. The resulting framework naturally
supports higher-order, deterministic, and nondeterministic programs
can capture both may- and must-style function summaries~\cite{GN+10},
and can be applied to a range of verification tasks, including
...
necessary. We then introduce a \emph{resource algebra} to compose and
destruct these capabilities in a principled way.  This approach
enables us to handle context disjointness analogously to the way
bunched implications~\cite{OHP99} handle resource separation, lifting
the additive/multiplicative conjunction distinction from the level of
...
\item We introduce \emph{context types}, a bunched modal type system
  ...
  with nondeterministism (Section~\ref{sec:extension}), where the
