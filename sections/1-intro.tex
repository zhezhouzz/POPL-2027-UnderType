...
example, several recent works~\cite{ReverseHoareLogic, OHP19, ZDS23}
have studied ``incorrectness'' systems designed to prove the existence
of witnesses of buggy program behavior, with a goal of avoiding the
overwhelming number of false positives reported by typical bug
detection tools.  The question of how to develop type systems
targeting reachability properties, however, has been largely
...
The intuitive meaning of each modality subtly changes when a
...
The paper makes the following contributions\footnote{The type system,
  logic, and metatheory presented in the paper are mechanized in Rocq,
  and presented as supplemental material.}:
...
\item We interpret the capability of a context to make angelic or
  ...
  express compositional operations over this
  capability. 
...
Section~\ref{sec:meta} defines a denotational semantics for this type
system via a bunched implication resource capability algebra along
with its metatheory.  Section~\ref{sec:extension} considers an
extension of the core language with non-determinism, along with
refinements to the logic and denotational semantics necessary to
support this new capability.  Section~\ref{sec:case} presents case
studies of application instances for which context types are
particularly well-aligned.  A discussion on potential ways to realize
an efficient typing algorithm as well as improved automation and
inference for this system is given in Section~\ref{sec:discussion}.
