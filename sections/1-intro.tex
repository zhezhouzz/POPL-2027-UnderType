...
inputs, its result is always a positive number smaller than its first
...
returns $x$. Our type system, in contrast, is capturing both kinds of
properties as types:
\begin{align}
  & x{:}\ort{\Nat}{\vnu > 0} \sarr y{:}\ort{\Nat}{\vnu > 0 } \sarr \ort{\Nat}{x \geq \vnu > 0} \tag{$\tau_a$}\\
  & x{:}\urt{\Nat}{\vnu > 0} \sarr y{:}\urt{\Nat}{\vnu > 0} \sarr \urt{\Nat}{\vnu > 0 } \tag{$\tau_b$}
\end{align}\noindent
...
%   yet.}
The first signature uses $\ort{b}{\phi}$ to express that the qualifier
$\phi$ should be interpreted as a safety constraint: a term that has
this type cannot produce any value outside the set of values the type
denotes.  Conversely, the second uses $\urt{b}{\phi}$ to capture a
reachability constraint: a term with this type must produce every
value found in the set the type denotes.

The intuitive meanings of each modality subtly change when a
...
when establishing the typing judgment
$x{:} \urt{b}{\phi} \vdash e ~:~\tau$, $x$ is treated
\emph{angelically} --- $x$ must be able to take on every value
consistent with $\phi$.
...
type it against the following stronger type signature:
\begin{align*}
...
be made regardless of the other.  While the result context of a
deterministic function is always dependent on its input context,
nondeterministic functions may produce results that are independent of
some inputs, a distinction that our system captures via disjoint
result contexts using $\ctwand$; we elaborate on this point in
\autoref{sec:extension}.
...
\item We introduce \emph{context types}, a bunched modal type system
  ...
  with nondeterminism (\autoref{sec:extension}), where the result
  context is allowed to be disjoint from the input context.
