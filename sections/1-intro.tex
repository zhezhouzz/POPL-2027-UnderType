...
deductive mechanism for \emph{compositionally verifying} that the
program is consistent with that specification.  Type systems have
traditionally targeted \emph{safety} properties, a focus embodied by
Milner's famous slogan that ``well-typed programs cannot {`go
wrong'}''~\cite{Milner+78}. Here, the typing judgement $\Gamma \vdash e
: \tau$ asserts that $e$ \emph{may} evaluate to many values (under any
...
ruling out certain executions, reachability analyses seek to certify
the existence of feasible executions.  Reachability analyses are a
well-established and practical technique for informing program
...
example, several recent works~\cite{GOS19,ReverseHoareLogic, OHP19,
  ZDS23} have studied deductive ``incorrectness'' reasoning frameworks
which can prove the existence of witnesses of safety violations, with
a goal of avoiding the high false positive rate reported by typical
bug detection tools.  The question of how to develop type systems for
reachability, however, has been largely unexplored.  Recent
works~\cite{RefinementTypeRefutations, CoverageType, Ill-types} that
have studied type-based reachability reasoning have either focused on
a particular property, e.g., checking completeness of test input
generators~\cite{CoverageType}, or a particular application, e.g.,
generating concrete counterexamples refuting refinement type
...
To unify these two views of types, this paper proposes a novel
semantic model that can simultaneously capture both overapproximations
(may-style) and underapproximations (must-style) of program behaviors,
...
indicating whether they should be interpreted as an over- or
under-approximate specification.  The type system naturally supports
both deterministic and nondeterministic higher-order programs, captures
both may- and must-style function summaries~\cite{GN+10}, and can be
applied to a range of verification tasks, including
incorrectness-style reasoning~\cite{OHP19} and proving completeness of
test-input generators~\cite{CoverageType}.
...
\begin{align}
  ...
  \tag{\Code{div}}\label{ex:div}
\end{align}
A client of \ref{ex:div} may want to know that (a) on positive
inputs, its result is never greater than its first argument (a safety
property); and (b) for every positive number $n$, there exist positive
inputs $x$ and $y$ such that \ref{ex:div} $x~ y = n$ (a reachability
property). Conventional refinement type systems, which interpret
refinements as overapproximations, can verify the former, but not the
latter: e.g., a function that always returns $1$ satisfies
(a), but not (b). Conversely, an underapproximate interpretation can establish (b),
but would also admit an implementation that always returns $y$, potentially violating (a). Our
type system, in contrast, captures both kinds of properties as types:\footnote{As we show later,
  these two types can be combined via intersection to simultaneously verify both
  requirements.}
\begin{align}
  & x{:}\ort{\Nat}{\vnu > 0} \sarr y{:}\ort{\Nat}{\vnu > 0 } \sarr \ort{\Nat}{x \geq \vnu} \tag{$\tau_a$}\label{ex:tauA}\\
...
for example, our type system treats any free occurrences of $x$
\emph{demonically}, since the surrounding environment is free to
choose some value of $x$ consistent with $\phi$ that makes $e$ violate
the constraints defined by $\tau$. Conversely, to establish
$x{:}\urt{b}{\phi} \vdash e : \tau$ $x$ is treated \emph{angelically}:
the surrounding environment is guaranteed to assign $x$ every value
consistent with $\phi$, allowing the type system to assume any such
value is available when verifying $e$ against $\tau$.

The availability of first-class functions adds additional
complications. Higher-order functions like
$x{:} \tau_x \vdash \lambda y. e : y{:}\tau_y \sarr \tau$, for
example, naturally expose two distinct contexts: the first,
$x{:} \tau_x$, captures the type of the free variable $x$, while the
second, $y{:} \tau_y$, provides assumptions about the function's
arguments at the point it is applied. Our system must manage these
contexts separately, because the instantiations of the free variables
and parameters of a function can occur at different points in a
program's evaluation. To illustrate, consider the following (failing)
type judgement:
\begin{align*}
    x{:}\urt{\Nat}{\vnu > 0} &\vdash  \lambda f. f\;x ~~:~~ f{:}(\urt{\Nat}{\vnu > 0}\sarr \urt{\Nat}{\vnu > 0}) \sarr \urt{\Nat}{\vnu > 0} \quad\judgfail
\end{align*}\noindent
The higher-order function being typed here takes a function $f$ and
...
the expression's closure, however, we cannot guarantee that an
arbitrary function supplied for $f$ will guarantee the reachability
requirement of this expression's result.  For example, applying this
expression to the function $\lambda y.x - y$ yields the constant
function that always returns $0$.
\begin{align*}
  & \zlet{x}{e_x}{\boxed{(\lambda f. f\;x)\;(\lambda y.x - y)}} \;\hookrightarrow^*
    \; \zlet{x}{e_x}{\boxed{x - x}} \;\hookrightarrow^*\; 0
\end{align*}\noindent
In contrast, applying $\lambda f. f\;x$ to the identity function,
$\lambda y.y$, yields the term $x$, which can be made to reach any
particular positive number $n$ by choosing $x$ to be $n$.  The problem
is that the ``captured context'' of the function $\lambda y.x - y$
contains $x$ \emph{and} that this function is supplied $x$ as its
argument. Thus, in order to assign $\lambda f. f\;x$ the desired
result type, the type of $f$ must enforce that its parameter $y$ can
be chosen independently of $x$. This requires our type system to
reason explicitly about dependencies between a function's parameters
and its captured variables when both are treated angelically.
...
reasoning.  The example below illustrates how a similar problem can
arise dynamically, during program execution. When the result of an
intermediate function application depends on a captured variable, it
can constrain the angelic choices available to subsequent applications
in a way that also breaks compositional reasoning:
\begin{align*}
  y{:}\urt{\Nat}{\vnu > 0} \vdash \lambda f. f(f\;y) : f{:}(\urt{\Nat}{\vnu > 0}\sarr \urt{\Nat}{\vnu > 0}) \sarr \urt{\Nat}{\vnu > 0} \quad \judgfail
\end{align*}\noindent
A counter-example to the validity of this judgement is given below:
{ \setlength{\jot}{-1pt} % Tighten line space for just this example
\begin{align*}
  & \zlet{x}{e_x}{\zlet{y}{e_y}{\boxed{(\lambda f. f\;(f\;y))\;g}}} \;\hookrightarrow^*\;
  \quad  \text{\hfill\textcolor{gray}{(where $g \doteq \lambda y.\zif{y = 1}{x}{2}$)}}
  \\& \zlet{x}{e_x}{\boxed{g\;(g\;1)}} \;\hookrightarrow^*\;  \hspace{1.38in}
  \text{\textcolor{gray}{\hfill(we assume angelically that $e_y \hookrightarrow^* 1$)}}
...
although the individual applications of $g$ in the original program
appear to satisfy its reachability specification, their composition
does not, again invalidating compositional reasoning.
...
that mediate demonic or angelic choices as necessary.  Concretely, a
capability associated with a typing context $\Gamma$ is a set of
environments consistent with $\Gamma$, representing the range of
variable bindings available for demonic or angelic choices.  We then
introduce a \emph{resource algebra} to compose and destruct these
capabilities in a principled way.  Specifically, the algebra ensures
that capabilities can only be combined and decomposed in a way that
faithfully reflects the dependency structure between variable bindings
in the typing context, as illustrated in the examples given above.
This approach enables us to handle context independence analogously to
the way bunched implications (BI)~\cite{OHP99} handle resource
...
This bunched type system enables the tracking of complex data-flow
dependencies and informs the angelic choices that are allowed in a
typing judgement.  Specifically, since contexts can be combined via
context combinators, the structure of a context is naturally a binary
tree whose internal nodes record how sub-contexts are combined,
enabling frame-based local reasoning.  For example, to establish that
\ref{ex:div} can produce all positive numbers, we can type it against
the following type signature that provides a more precise version of $\tau_b$
using $\ctwand$ to make explicit that $x$ and $y$ must be chosen independently:
\begin{align*}
  & \vdash \ref{ex:div} ~:~ x{:}\urt{\Nat}{\vnu > 0} \ctsarr y{:}\urt{\Nat}{\vnu > 0} \ctwand \urt{\Nat}{\vnu > 0} \quad \judgok
\end{align*}\noindent
...
The paper makes the following contributions:\footnote{The type system,
  logic, and metatheory have been fully mechanized in Rocq, and are
...
\item Building on this algebra, we define an associated modal
  \emph{context logic} that supports entangled and independent mixing
  of safety/demonic and reachability/angelic modalities.
...
example-driven tour of our type system.
Section~\ref{sec:context-typing} formalizes this informal overview in
terms of a typed $\lambda$-calculus core language.
...
nondeterminism, and describes supporting modifications to the logic
and denotational semantics.  Section~\ref{sec:case} presents several
case studies for which context types are particularly well-aligned.  A
brief discussion on potential ways to realize effective verification
tools based on this framework is given in
Section~\ref{sec:discussion}.  Related work and conclusions follow in
