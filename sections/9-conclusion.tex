...
which every type qualifier is equipped with a modality indicating
whether it should be interpreted as an over- or under-approximate
...
Our metatheory, mechanized in Rocq, establishes that a single
soundness theorem delivers both guarantees: every value produced by a
well-typed term satisfies its demonic types, and every value
prescribed by its angelic types is witnessed by some execution.

%% Bunched context combinators track the dependencies between variables
%% and determine when angelic choices may be made independently, a
