...
\autoref{fig:may-must-reach} must return \Code{1} on some
arguments, for example, the program analysis of
\citet{CompositionalMayMust} uses a not-may summary analogous to the
signature of \Code{g} to conclude calling it will never produce
the \Code{0} value needed for \Code{f} to yield
\Code{1}. As we shall see shortly, our type system also
