...
\paragraph{Combining Type Contexts}
...
modalities afford us different reasoning capabilities: when verifying
the reachability properties of a term, for example, an angelic
refinement lets us \emph{choose} a favorable assingment for a variable
in that term, while a demonic refinement allows us to \emph{rule out}
certain assignments to that variable establishing that a term is safe.
...
\end{align}\noindent
Both contexts use \Code{nat\_gen ()} to (nondeterministicly) generate
an aribtrary value for $x$. The first context then sets $y$ based on
...
affords different capabilities to the combined context.  Informally,
$\Gamma_1 \ctstar \Gamma_2$ means that the capabilities of $\Gamma_1$
and $\Gamma_2$ are independent; when either context includes an
angelic type binding, our type system is free to ``choose'' a
favorable assignment for the corresponding variable. By first choosing
an arbitrary positive value for $x$ and then choosing 0 for $y$, we
can establish the following judgment:
\begin{align*}
  &x{:}\urt{\Nat}{\vnu > 0} \ctstar y{:}\urt{\Nat}{\vnu > 0} \vdash x
    - y : \urt{\Nat}{\vnu > 0} \;\judgok
\end{align*}\noindent
The $\ctcomma$ operator affords us no such freedom, however,
preventing us from establishing a similar judgment:
\begin{align*}
  &x{:}\urt{\Nat}{\vnu > 0} \ctcomma y{:}\urt{\Nat}{\vnu > 0} \vdash x - y : \urt{\Nat}{\vnu > 0} \;\judgfail
\end{align*}\noindent
\BD{I'm not sure how to frame the ``capabilities'' provided by
  $\ctcomma$... How do we argue that we cannot prove the previous
  claim?}


The environment $\EnvirDep$
cannot satisfy the disjointness requirement, since once $x$ is bound,
