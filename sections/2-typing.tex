...
\end{align*}\noindent
\emph{Entangled} conjunction, $\ctcomma$, affords us no such freedom,
however, preventing us from establishing a similar judgment:
\begin{align*}
...
\BD{I'm not sure how to frame the ``capabilities'' provided by
  $\ctcomma$... Mechanically, how does our type system prevent us from
  typing the previous example?}  The hole in \ref{ex:EnCon} can be typed
under this latter typing context: once $x$ is bound, the value of $y$
now depends on it.

The angelic and demonic refinements in a type context can also be
entangled in a way that impacts its capabilities, as the following
judgments illustrate:
\begin{align*}
  &x{:}\urt{\Nat}{\top} \ctcomma y{:}\ort{\Nat}{\vnu > 0} \vdash x - y : \urt{\Nat}{\I{even}(\vnu)} \;\judgfail
  \\&x{:}\urt{\Nat}{\top} \ctstar y{:}\ort{\Nat}{\vnu > 0} \vdash x - y : \urt{\Nat}{\I{even}(\vnu)} \;\judgok
\end{align*}\noindent
...
context. Accordingly, the expression $x - y$ always evaluates to $1$,
and cannot reach every even number. Contrast this with a variant of
this context that instead assigns \emph{some} constant value to $y$:
\begin{align}
...
guarantee that the modalities of $x$ and $y$ hold \emph{at the same
  time}. \BD{Can we summarize this point using an example, e.g.,
  $\Gamma \not\vdash (x, y) {:}(\tau_x, \tau_y)$ ? }
\footnote{Unlike a linear type system, which would \emph{consume} the
  capability to angelically assign $x$ when producing the capability
  to angelically assign $y$, our system instead \emph{entangles} these
  capabilities, allowing the body expression $e$ to use both $x$ and
  $y$ while still tracking their dependency. This formulation aligns
  with the intuition of bunched typing~\cite{BunchedType}. \BD{I don't
    think the reader is going to appreciate this difference unless
    they have a better intuition of the capabilities afforded by
    $\ctcomma$.}}  For example,
\begin{align*}
...
\paragraph{Function Types} Entangled ($\ctsarr$) and independent
