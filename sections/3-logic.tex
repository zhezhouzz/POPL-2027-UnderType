...
and $\sigma_2 \in
r_2$; it combines capabilities over disjoint variable domains.
The
...
\begin{definition}[Context Logic]\label{def:context-logic}
  ...
  \item $r \models \demonic P$ iff $\exists r', r \sqsubseteq r'$ such that $r' \models P$;
  \item $r \models \angelic P$ iff $\exists r', r' \sqsubseteq r$ such that $r' \models P$;
...
additional variable bindings, it may place fewer constraints on the
existing bindings.  Dually, the angelic modality $\angelic P$ holds at
$r$ if some \emph{sub-capability} $r' \sqsubseteq r$ satisfies $P$;
...
\begin{example}[Approximation Modalities]\label{ex-approximation-modalities}
  ...
  no extension of $r_x$ satisfies $\Code{Atom}(x = 1)$ exactly (since
  $r_x$ already contains $[x \mapsto 2]$, any extension violates
