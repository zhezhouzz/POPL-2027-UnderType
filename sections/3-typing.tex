\begin{figure}[!t]
  {\small
    \begin{alignat*}{2}
      ...
      \\\text{\textbf{Built-in Operations}}& \quad & \primop ::= \quad & d ~|~ {+} ~|~ {-} ~|~ {==} ~|~ {<} ~|~ {\leq} ~|~ ...
...
of contexts cannot share any varible bindings (\textsc{WfStar}). In
...
typing contexts and types; we elide these assumptions for clarity. Our
rules use the intersection of an angelic and demonic refinement to
assign a precise (singleton) type to a term: \textsc{T-Const}, for
example, assigns the constant $c$ the type
$\ort{b}{\vnu = c} \ctinter \urt{b}{\vnu = c}$. The typing rules for
let-expressions, lambda functions, and applications all have entangled
and independent versions. In contrast to a standard bunched type
system, the entangled version of the let rule does not require an
explicit additive conjunction in the type context. Instead,
...
$\prec$. As a consequnce, the first parameter of a recursive function
