\section{Supporting Nondeterministism (Suresh)}\label{sec:extension}
...
($\ctstar$) conjunction in the second-to-first line.  This shift
follows from the denotational semantics. By
Definition~\ref{def:context-denotation}, the dependent conjunction
``$\ctcomma$'' maps to additive conjunction ``$\chland$''
and the disjoint conjunction ``$\ctstar$'' maps to
multiplicative conjunction ``$\chstar$''. The shift therefore
requires showing:
\begin{align*}
  \denot{\Code{nat\_gen}{:}\ort{\Unit}{\top} \ctsarr \urt{\Nat}{\top}} \chland \denot{x{:}\urt{\Nat}{\top}} \chland \denot{y{:}\urt{\Nat}{\top}} \models\\
  \denot{\Code{nat\_gen}{:}\ort{\Unit}{\top} \ctsarr \urt{\Nat}{\top}} \chstar \denot{x{:}\urt{\Nat}{\top}} \chstar \denot{y{:}\urt{\Nat}{\top}}
\end{align*}  
This holds because the denotation of $\Code{nat\_gen}$'s type is
guarded by the demonic modality and, as established above, this means
the capability for $\Code{nat\_gen}$ can be freely separated from
those of $x$ and $y$ via ``$\chstar$''.
...
covers both.  Under a call-by-value semantics, results in supplying either
$\zlam{x}{\Unit}{\true}$ or $\zlam{x}{\Unit}{\false}$
as the argument to $f$; both subsequent calls then return the same
