...
\begin{definition}[Contextual Capability]\label{def:contextual-capability}
  ...
  domain.  We use $r$ to range over contextual capabilities and $\Res$
  as the set of all well-formed contextual capabilities; we assume
  throughout that all capabilities are well-formed.
\end{definition}
...
unions $\sigma_1 \cup \sigma_2$ with $\sigma_1 \in r_1$
and $\sigma_2 \in
r_2$.
...
The
\emph{sum} operation, $r_1 + r_2$, defined when $\Dom{r_1} = \Dom{r_2}$, is
...
\{\emptyset\}$, the tuple $(\Res, +, \times, \mathbf{1})$
...
\begin{definition}[Context Logic]\label{def:context-logic}
  ...
  \item $r \models \forallM{x} P$ iff $\forall [x \mapsto v_x] \in \projectTo{r}{\{x\}}.\; \fiberOver{r}{[x \mapsto v_x]} \models P[x \mapsto v_x]$.\footnote{
...
$\chforall x.\, P$ and $\chexists x.\, P$
use the refinement
relation $\sqsubseteq$ as the Kripke accessibility relation: truth at
$r$ requires truth at all $r'$ such that $r \sqsubseteq r'$, i.e., at all extensions
...
The demonic modality $\demonic P$ holds at $r$ if there exists an upper bound $r'$ (i.e., $r \subseteq r'$) that satisfies
$P$; this models an overapproximation since $r'$ extends $r$ with
additional environments.  Dually, the angelic modality $\angelic P$ holds at
$r$ if some lower bound $r' \subseteq r$ satisfies $P$;
this models an underapproximation since $r'$ witnesses a specific
subset of the environments in $r$.
We would like to highlight that the refinement relation $\sqsubseteq$ can also be used to extend the entailment of demonic and angelic modalities. Consider the following example:
\begin{align*}
\begin{array}{c @{\hspace{1.5em}} c @{\hspace{1.5em}} c}
  {\{[x \mapsto 1], [x \mapsto 2]\}} & \sqsubseteq & {\boxed{r' \doteq \{[x \mapsto 1; y \mapsto 2], [x \mapsto 2; y \mapsto 7]\}}} \\[0.6em]
  \vsubseteq & & \vsubseteq \\[0.6em]
  {\boxed{r \doteq \{[x \mapsto 1]\}}} & \sqsubseteq & {\{[x \mapsto 1; y \mapsto 2]\}}
\end{array}
\end{align*}\noindent
For any angelic formula $\angelic P$, if we have $r \models \angelic P$, then we also have $r' \models \angelic P$. We have a similar result for the demonic formula $\demonic P$ (i.e., $r \models \demonic P$ implies $r' \models \demonic P$) as illustrated in the following example:
\begin{align*}
\begin{array}{c @{\hspace{1.5em}} c @{\hspace{1.5em}} c}
  {\{[x \mapsto 1]\}} & \sqsubseteq & {\boxed{r' \doteq \{[x \mapsto 1; y \mapsto 2]\}}} \\[0.6em]
  \vsupseteq & & \vsupseteq \\[0.6em]
  {\boxed{r \doteq \{[x \mapsto 1], [x \mapsto 2]\}}} & \sqsubseteq & {\{[x \mapsto 1; y \mapsto 2], [x \mapsto 2; y \mapsto 7]\}}
\end{array}
\end{align*}\noindent
Note that the subset relation is reversed; however, the refinement relation is not.
...
\ctoplus \tau_2$ introduced in Section~\ref{sec:context-typing}, where a
...
range of $x$ expressed by other conjuncts.  The binding reference operator
...
\begin{example}[Binding Reference Operation]\label{ex-variable-reference}
  Consider the dependent angelic context
  \[x{:}\urt{\Nat}{1 \leq \vnu
...
the context logic specified above, and state the main soundness
theorem.  We write $\denot{\tau}_\Sigma : \Code{Term} \to
\Code{Formula}$ for the type denotation under basic type context
$\Sigma$, where $r \models \denot{\tau}_\Sigma\;e$ means that $e$
inhabits type $\tau$ under capability $r$.  We omit $\Sigma$ when it
...
\end{align*}
The binding reference operator $\forallM{\Dom{\Sigma}}$ is necessary here;
...
\end{align*}
Thus, the binding reference operator $\forallM{\Dom{\Sigma}}$ fixes the
bindings of all context variables before evaluating the atomic
reduction predicate, ensuring that the reduction $\mstep{e}{x}$ is
checked under each specific environment in the current capability
...
\begin{definition}[Type Denotation]\label{def:type-denotation}
  \begin{align*}
    \denot{\ort{b}{\phi}}_\Sigma\;e &\doteq \denot{\mstep{e}{\vnu}}_\Sigma \chimpl \forallM{\Code{FV}(\ort{b}{\phi})}\,\demonic\,\Code{Atom}(\phi) \quad \text{($\vnu$ fresh)}
    \\\denot{\urt{b}{\phi}}_\Sigma\;e &\doteq \denot{\mstep{e}{\vnu}}_\Sigma \chimpl \forallM{\Code{FV}(\urt{b}{\phi})}\,\angelic\,\Code{Atom}(\phi) \quad \text{($\vnu$ fresh)}
...
This theorem states that the typing judgement $\Gamma \vdash e : \tau$
is sound with respect to the denotational semantics: any capability
that satisfies the denotation of $\Gamma$ also satisfies the
denotation of $\tau$ applied to $e$.
...
The soundness theorem instantiates the fundamental theorem at the
empty context. For a closed well-typed term $e$, the capability $\{[x
  \mapsto v] \mid \mstep{e}{v}\}$, which collects all values $e$ can
reduce to, each bound to a fresh variable $x$, satisfies the
denotation of $\tau$. This has two concrete consequences depending on
the modality of $\tau$: if $\tau$ is a demonic type, then every
value that $e$ reduces to satisfies $\tau$'s qualifier (safety); if $\tau$
is an angelic type, then every value prescribed by $\tau$'s
qualifier is witnessed by some reduction of $e$ (reachability).
