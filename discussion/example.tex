...
\paragraph{Under-Approximation Helps Refine Correctness Specification} The second implementation is indeed correct, and under-approximation will fail to prove the reachability of any potential buggy path. For example, it fails to prove that $\Code{h1\;::\;merge\;t1\;(h2\;::\;t2)}$ is sorted:
\begin{align*}
    \not\vdash \Code{merge}& : \Code{h1}{:}\ort{\Int}{\top} \garr \Code{t1}\ort{il}{\I{sorted}(\vnu)} \sarr \Code{l2}{:}\ort{il}{\I{sorted}(\vnu)} 
    \\&\sarr \ort{il}{\I{sorted}(\Code{h1}::\vnu)} \tag{$\tau_\Code{over}$}
\end{align*}\noindent
In order to get a more precise type than $\tau$, the type $\tau_\Code{over}$ above provide a good template. It shows that under some condition, $\Code{h1}$, $\Code{t1}$, and $\Code{l2}$ will leads unsorted list. If we can figure this condition out and rule out it from $\tau_\Code{over}$, then we get a refined over-style specification. Concretely, we preform underapproximation typecheck from the given type
\begin{align*}
    \not\vdash \Code{merge}& : \Code{h1}{:}\urt{\Int}{\top} \garr \Code{t1}\urt{il}{\I{sorted}(\vnu)} \sarr \Code{l2}{:}\urt{il}{\I{sorted}(\vnu)} 
    \\&\sarr \ort{il}{\neg\I{sorted}(\Code{h1}::\vnu)} \tag{$\tau_\Code{under}$}
\end{align*}\noindent
The goal is (1) checks weather unsorted result is reachable (2) refine the input domain to get a more precise ``erroneous precondition''.

Consider the control flow on line $3$ where $\Code{t1}$ is empty list, the merge result is just $\Code{l2}$, and we want to prove $\neg\I{sorted}(\Code{h1::l2})$. It easily to know the unsorted result is reachable, and the precondition $\Code{h1}{:}\urt{\Int}{\top}$ and be narrow down into $\Code{h1}> \I{hd}(\Code{l2})$ (according to the definition of $\I{sorted}$ on line $21$):
\begin{align*}
    \not\vdash \Code{merge}& : \Code{h1}{:}\urt{\Int}{\top} \garr \Code{t1}\urt{il}{\I{sorted}(\vnu) \land \Code{h1} > \I{hd}(\vnu)} \sarr \Code{l2}{:}\urt{il}{\I{sorted}(\vnu)} 
    \\&\sarr \ort{il}{\neg\I{sorted}(\Code{h1}::\vnu)} \tag{$\tau_\Code{under}'$}
\end{align*}\noindent

Similarly, consider the control flow on line $4$, we get another erroneous precondition is $\Code{h1} > \I{hd}(\Code{t2})$.
\begin{align*}
    \not\vdash \Code{merge}& : \Code{h1}{:}\urt{\Int}{\top} \garr \Code{t1}\urt{il}{\I{sorted}(\vnu)} \sarr \Code{l2}{:}\urt{il}{\I{sorted}(\vnu) \land \Code{h1} > \I{hd}(\vnu)} 
    \\&\sarr \ort{il}{\neg\I{sorted}(\Code{h1}::\vnu)} \tag{$\tau_\Code{under}''$}
\end{align*}\noindent

Now we rule out these two underapproximation types $\tau_\Code{under}'$ and $\tau_\Code{under}''$ from $\tau_\Code{over}$ and lead the following type:
\begin{align*}
