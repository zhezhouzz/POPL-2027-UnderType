...
\begin{align*}
    x{:}\ort{\Int}{\vnu > 1} \sarr \ort{\Int}{\vnu > 0} \tag{$\tau$}
\end{align*}\noindent
...
\begin{enumerate}
    \item The program $\Code{f1}$ is consistent with the type $\tau$, and can be type checked by standard refinement type system.
    \item The program $\Code{f2}$ is not consistent with the type $\tau$, one possible counterexample is $\Code{f2\;2 = 0}$. Thus standard refinement type system will fail to type check it. It can be type checked by underapproximated type system.
    \begin{align*}
        \vdash \Code{f2} : \urt{\Int}{\vnu > 1} \sarr \ort{\Int}{\vnu \leq 0} \tag{$\tau_{\neg}$}
    \end{align*}\noindent
    \item The program $\Code{f3}$ is not consistent with the type $\tau$, one possible counterexample is $\Code{f3\;3 = 0}$. If we want to reuse the under-type of $\Code{f2}$ in the underapproximation reasoning, we need
    \begin{align*}
        \vdash \Code{f1} : x{:}\urt{\Int}{\vnu > 1} \sarr \urt{\Int}{\vnu > 1}
    \end{align*}\noindent
    which says all possible interger greater than $1$ is reachable.
    \item The program $\Code{f4}$ is consistent with the type $\tau$, but cannot be type checked by standard refinement type system. It needs a stronger type:
    \begin{align*}
        x{:}\ort{\Int}{\textcolor{red}{\vnu > 0}} \sarr \ort{\Int}{\vnu > 0} \tag{$\tau'$}
    \end{align*}\noindent
    Note that the recursive application has argument $x - 1$ that can be $1$ which is outside the precondition of the type $\tau$.
\end{enumerate}
...
\begin{align*}
    \Code{merge} : \ort{il}{\I{sorted}(\vnu)} \sarr \ort{il}{\I{sorted}(\vnu)} \sarr \ort{il}{\I{sorted}(\vnu)} \tag{$\tau$}
\end{align*}\noindent
...
\begin{align*}
    ...
    \\&\ort{il}{\I{sorted}(\vnu) \land \I{elemSet}(l_1) \cup \I{elemSet}(l_2) = \I{elemSet}(\vnu)} \tag{$\tau_\Code{membership}$}
\end{align*}\noindent
...
\begin{align*}
    \Code{merge}& : \urt{il}{\I{sorted}(\vnu) \land \vnu \not= [] } \sarr \urt{il}{\I{sorted}(\vnu)} \sarr \ort{il}{\neg \I{sorted}(\vnu)}
    \\\Code{merge}& : \ort{il}{\vnu = [] } \sarr y{:}\ort{il}{\I{sorted}(\vnu)} \sarr \ort{il}{\vnu = y}
    \\\Code{merge}& : x{:}\ort{il}{\I{sorted}(\vnu)} \sarr \ort{il}{\vnu = []} \sarr \ort{il}{\vnu = x}
\end{align*}\noindent
...
\paragraph{Under-Approximation Helps Refine Correctness Specification} The second implementation is indeed correct, and under-approximation will fail to prove the reachability of any potential buggy path.
\begin{align*}
    \not\vdash \Code{merge}& : \Code{l1}{:}\urt{il}{\I{sorted}(\vnu)} \sarr \Code{l2}{:}\urt{il}{\I{sorted}(\vnu)} \sarr \ort{il}{\neg \I{sorted}(\vnu)} \tag{negation of $\tau$}
\end{align*}\noindent
Specifically, it fails to show that $\Code{h1\;::\;merge\;t1\;(h2\;::\;t2)}$ is unsorted (line 14 in Figure \ref{fig:motivation}), or:
\begin{align*}
    \Code{h1}{:}\urt{\Int}{\vnu \leq \Code{hd(t1)}} \not\vdash \Code{merge}& : \Code{t1}\urt{il}{\I{sorted}(\vnu)} \sarr \Code{l2}{:}\urt{il}{\I{sorted}(\vnu)} 
    \\&\sarr \ort{il}{\neg (\Code{h1} \leq \Code{hd}(\vnu) \land \I{sorted}(\vnu))}
\end{align*}\noindent
The qualifier for $\Code{h1}$ and the return type can be derived directly from the definition of $\I{sorted}$.
Since $\tau$ already requires that $\I{sorted}(\vnu)$ holds, this type judgment provides a template $\square \leq \Code{hd}(\vnu)$ (where $\Code{h1}$ does not belong to the parameters of $\Code{merge}$, so we use a placeholder $\square$) to indicate how we can refine the return type of $\tau$ to obtain a more precise ``postcondition.'' We can simply enumerate several possible type-safe candidates for the placeholder $\square$: $\Code{hd(t1)}$ and $\Code{hd(l2)}$ from the two parameters in the $\Code{merge}$ function. According to the qualifier of $\Code{h1}$ (i.e., $\Code{h1} \leq \Code{hd(t1)}$), it is easy to see that $\Code{hd(t1)}$ is the correct instantiation of $\square$, leading to the following refined type:
\begin{align*}
    \Code{merge}& : \Code{l1}{:}\ort{il}{\I{sorted}(\vnu)} \sarr \Code{l2}{:}\ort{il}{\I{sorted}(\vnu)} \sarr \ort{il}{\Code{hd(t1)} \leq \Code{hd(\vnu)} \land \I{sorted}(\vnu)} \tag{$\tau_1$}
\end{align*}\noindent
However, the overapproximation typecheck still fails for the refined type $\tau_1$. Naive search may easily get lost here, as we do not know if the current refined candidate type is on the correct track—we can either give up the current refined type or continue searching, which leads to an exponential search space. Fortunately, we can still use under-approximation typechecking to help us make the decision. For example, we find that the negation of $\tau_1$ is still unreachable:
\begin{align*}
    \not\vdash \Code{merge}& : \Code{l1}{:}\urt{il}{\I{sorted}(\vnu)} \sarr \Code{l2}{:}\urt{il}{\I{sorted}(\vnu)} \sarr \ort{il}{\neg (\Code{hd(l1)} \leq \Code{hd}(\vnu) \land \I{sorted}(\vnu))} \tag{negation of $\tau_1$}
\end{align*}\noindent
It means the refined type $\tau_1$ is valuable to keep.
Specifically, it fails to show that $\Code{h2\;::\;merge\;(h1\;::\;t1)\;t2}$ is unsorted (line 15 in Figure \ref{fig:motivation}), or:
\begin{align*}
    \Code{h2}{:}\urt{\Int}{\vnu \leq \Code{hd(t2)}} \not\vdash \Code{merge}& : \Code{l1}\urt{il}{\I{sorted}(\vnu)} \sarr \Code{t2}{:}\urt{il}{\I{sorted}(\vnu)} 
    \\&\sarr \ort{il}{\neg (\Code{h2} \leq \Code{hd(l1)} \land \Code{h2} \leq \Code{hd}(\vnu) \land \I{sorted}(\vnu))}
\end{align*}\noindent
Similarly, we can find that both $\I{sorted}(\vnu)$ and $\Code{h2} \leq \Code{hd(l1)}$ hold (according to the condition on line 14). We again get the refinement template $\square \leq \Code{hd(\vnu)}$, but now $\square$ should be instantiated as a different term, $\Code{hd(l2)}$. This leads to the following refined type:
\begin{align*}
    \Code{merge}& : \Code{l1}{:}\ort{il}{\I{sorted}(\vnu)} \sarr \Code{l2}{:}\ort{il}{\I{sorted}(\vnu)} \sarr \ort{il}{\Code{hd(l2)} \leq \Code{hd(\vnu)} \land \Code{hd(l1)} \leq \Code{hd(\vnu)} \land \I{sorted}(\vnu)} \tag{$\tau_2$}
\end{align*}\noindent
At this point, $\tau_2$ is sufficient to make the typecheck succeed. 

\paragraph{Discussion} Note that the refined specification $\tau_2$ is much weaker than the one considering membership; also, we do not add any new predicates (e.g., $\I{elemSet}$) to the specification, and no new lemmas are required.
