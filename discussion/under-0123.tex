...
Here is a more complicate example where these two design may lead unsound results. Consider the function $g$ has the type $\urt{\Int}{\top} \sarr \tau$, which requires that arguement must cover all possible integers (or, one secrete value can lead $y{:}\tau$ holds). In either later control flow, we cannot say $y$ has type $\tau$, $x$ cannot cover all possible integers anymore. Even $\tau$ may not refer $x$, but it still depends on $x$, and can potentially be cancelled out by later constraint on $x$.

In summary, with underapproximation, we need to design a better way express the context change along control flow, maybe track the dependency between variables is a good idea.

\section{Underapproximation Only Case}

$\Code{low}$ and $\Code{high}$ should be at the same level (instead of one depending on the other), and we can freely refine one depending on the other according to the program under verification, but only perform this refinement as needed. I call this relation ``entanglement'', and I prefer to encode them as follows:
\begin{align*}
